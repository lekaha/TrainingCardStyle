...
public class SwipeDismissListViewTouchListener implements SwipeOnTouchListener {
    ...
    private boolean handleMoveEvent(final MotionEvent motionEvent) {
        ...
        if (mTouchChildTouched && 
        		!mDisallowSwipe && Math.abs(deltaX) > mSlop && Math.abs(deltaX) > Math.abs(deltaY)) {
            mSwiping = true;
            mListView.requestDisallowInterceptTouchEvent(true);
        ...
        if (mSwiping) {
            ...
            mCurrentDismissData.view.setAlpha(Math.max(0f, 
    ...
    /**
     * Handling CancelEvent.
     * 
     * @param motionEvent "Unused Parameters"
     * @return
     */
